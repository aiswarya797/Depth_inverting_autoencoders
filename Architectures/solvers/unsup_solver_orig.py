...
from models.BLT_models import multi_VAE, SB_decoder, spatial_broadcast_decoder,shallow_autoencoder,syn_encoder_decoder

from solvers.visuals_mod import traverse_z,construct_z_hist, plotsave_tests,find_top_N,plotsave_tests_loss_list,analyse_Z
...
class Solver_unsup(object):
    def __init__(self, args):
        ...
        #net = syn_encoder_decoder(self.encoder,self.decoder,self.z_dim_bern,self.z_dim_gauss,self.n_filter,self.nc,
                        #self.n_rep,self.sbd, self.kernel_size, self.padding, self.AE)
    ...
    def train(self):
        ...
        while not out:
            for sample in self.train_dl:
               ...
               # print(x_recon_list.type())
                x_recon = x_recon_list[-1]
                #x_recon = x_recon_list
                ...
                if self.global_iter ==1:
                    #print(self.z_prev.data[50])
                    #_z = self.z_prev.data[50]
                    #rev = np.flip(_z.numpy(),0).copy()
                    #_z = torch.from_numpy(rev)
                   # print(_z)
                    if self.gnrl_dl != 0:
                ...
                if self.global_iter%self.save_step == 0:  #this step makes the reconstruction plots for 30 images
                    ...
                    net_copy.to('cpu')
                    #analyse_Z(net_copy,self.gnrl_data, self.output_dir, self.global_iter)
